...
class CryptoRecommender:
    def __init__(self, use_cache_only=False, use_proxies=False, refresh_cache=False):
        ...
        if use_cache_only:
            ...
            logging.info("使用强制刷新模式")
            print("已激活强制刷新模式 - 忽略现有缓存，始终获取最新数据")
        else:
    ...
    def generate_recommendations(self, current_date, limit=None, sort_by='score', save_timestamp=True, save_all_data=False, email_addresses=None, high_score_threshold=50):
        ...
        # 初始化推荐列表
        recommendations = []
        high_score_recommendations = []  # 存储高分推荐
            ...
            try:
                ...
                if buy_decision:
                    ...
                    if recommendation_data.get('score', 0) >= high_score_threshold and email_addresses:
                        ...
                        # 准备单个币种的邮件内容
                        coin_email_subject = f"发现高评分蔬菜: {recommendation_data['symbol']} (评分: {recommendation_data['score']})"
                        
                        coin_email_content = f"""
发现一个评分较高的蔬菜!

蔬菜代码: {recommendation_data['symbol']}
名称: {recommendation_data['name']}
当前价格: {recommendation_data['price']}
市值: {recommendation_data['market_cap']}
24小时价格变化: {recommendation_data['price_change_24h']}%
7天价格变化: {recommendation_data['price_change_7d']}%
评分: {recommendation_data['score']}
推荐原因: {', '.join(recommendation_data['reason'])}
...
此为自动发送的高评分蔬菜通知。
                            ...
                            if email_address and email_address.strip():
                                try:
                                    send_results_to_qq_email(
                                        ...
                                        files=None,  # 单币种通知不附加文件
                                        use_obfuscation=True
                                    )
                                    logger.info(f"已发送高评分蔬菜 {recommendation_data['symbol']} 通知到 {email_address}")
                                except Exception as e:
                                    logger.error(f"发送高评分通知邮件失败: {e}")
        ...
        print(f"高评分蔬菜(>={high_score_threshold}分): {len(high_score_recommendations)} 种")
    ...
    def simulate_daily_reminders(self, days=7, limit=None, sort_by='score', save_timestamp=True, save_all_data=False, email_addresses=None, high_score_threshold=50):
        """模拟每日菜价提醒"""
...
def main():
    try:
        ...
        parser.add_argument('--refresh-cache', action='store_true', help='强制刷新缓存，始终获取最新数据')
        ...
        parser.add_argument('--high-score', type=int, default=50, help='高评分通知阈值，得分超过此值的蔬菜将立即发送通知')
        ...
        email_addresses = []
        if args.qq_email:
            email_addresses = [email.strip() for email in args.qq_email.split(',') if email.strip()]
        ...
        results = recommender.simulate_daily_reminders(
            ...
            email_addresses=email_addresses,
            high_score_threshold=args.high_score
        )
        ...
        if results:
            ...
            print(f"其中高评分蔬菜(>={args.high_score}分): {results.get('high_score_count', 0)} 个")
            ...
            if not args.no_save and results.get('recommendations_count', 0) > 0:
                ...
                csv_file = result_files.get('csv_file', '')
...
高评分蔬菜(>={args.high_score}分): {results.get('high_score_count', 0)} 种
