...
class CryptoRecommender:
        ...
        if self.use_cache_only:
            if self._is_cache_valid(cache_path):
                ...
                try:
                    ...
                    logger.error(f"仅缓存模式错误: 无法读取缓存文件 {cache_path}: {e}")
                    return None
            else:
        ...
        try:
            ...
            if is_short_term and 'volume' in df.columns:
                for col in ['volume_ma5', 'volume_ma10', 'volume_change', 
                         'momentum', 'obv', 'bias5', 'bias10', 'bias20']:
                    if col in df.columns:
                        indicators[col] = df[col].iloc[-1]
            ...
            indicators['price_above_ma20'] = df['close'].iloc[-1] > indicators['ma20']
    ...
    def generate_recommendations(self, current_date, limit=None, sort_by='score', save_timestamp=True, save_all_data=False, short_term_analysis=True):
        ...
        limit -- 分析的蔬菜数量限制
        sort_by -- 排序方式
        save_timestamp -- 是否保存带时间戳的结果
        save_all_data -- 是否保存所有原始数据
        ...
        if btc_risk:
            logger.info(f"当前BTC风险级别: {btc_risk['level']} (得分: {btc_risk['score']})")
        else:
            logger.warning("无法获取BTC风险分析，将使用默认风险设置")
            btc_risk = {'level': '中等', 'score': 50}
        
        # 根据BTC风险调整买入策略
        if btc_risk['level'] in ['高', '极高']:
            logger.info("由于BTC风险较高，将提高买入门槛")
        ...
        # 初始化推荐列表
        recommendations = []
        long_term_recommendations = []  # 长线推荐
        short_term_recommendations = []  # 短线推荐
        ...
        for i, coin in enumerate(coins):
            try:
                ...
                correlation = None
                if i > 0:  # 跳过BTC自身
                    correlation = self.calculate_correlation_with_btc(coin_id)
                
                # 获取日线历史价格数据（长期分析）
                long_history = self.fetch_price_history(coin_id, days=30)
                
                # 如果短线分析启用，获取小时级数据
                short_history = None
                if short_term_analysis:
                    short_history = self.fetch_price_history(coin_id, days=7, interval='hourly')
                
                # 获取多时间维度价格变化
                price_changes = self.fetch_price_changes(coin_id)
                
                # 如果无法获取历史数据，跳过该币种
                if long_history is None or long_history.empty:
                    progress_bar.update(1)
                    continue
                    
                # 计算长期技术指标
                long_indicators = self.calculate_technical_indicators(long_history)
                
                # 计算短期技术指标
                short_indicators = None
                if short_term_analysis and short_history is not None and not short_history.empty:
                    short_indicators = self.calculate_technical_indicators(short_history, is_short_term=True)
                
                # 分析长期市场趋势
                long_market_trend = self.analyze_market_trend(long_history)
                
                # 分析短期市场趋势
                short_market_trend = None
                if short_term_analysis and short_history is not None and not short_history.empty:
                    short_market_trend = self.analyze_market_trend(short_history)
                
                # 将价格变化数据添加到币种数据中
                if price_changes:
                    for period, change in price_changes.items():
                        if period not in ['current_price', 'total_volume', 'market_cap']:
                            key = f'price_change_percentage_{period}'
                            coin[key] = change
                
                # 确定是否推荐购买（长期分析）
                long_buy_decision, long_recommendation = self.determine_buy_action(
                    coin, 
                    long_indicators, 
                    long_market_trend, 
                ...
                # 如果有短期数据，确定短期推荐
                short_buy_decision = False
                short_recommendation = None
                if short_term_analysis and short_indicators and short_market_trend:
                    short_buy_decision, short_recommendation = self.determine_short_term_buy_action(
                        coin,
                        short_indicators,
                        short_market_trend,
                        verbose=(i < 3)
                ...
                # 根据BTC风险调整推荐
                if btc_risk['level'] in ['高', '极高'] and i > 0:  # 不调整BTC自身
                    # 降低推荐概率
                    risk_adjustment = min(1.0, btc_risk['score'] / 100 * 1.5)  # 风险调整因子
                    
                    # 如果与BTC高度相关，在高风险时降低推荐概率
                    if correlation and correlation.get('returns_correlation', 0) > 0.7:
                        long_buy_decision = long_buy_decision and (random.random() > risk_adjustment)
                        if short_buy_decision:
                            short_buy_decision = short_buy_decision and (random.random() > risk_adjustment)
                            
                    # 对于Beta值高的币种（波动性大于BTC），降低推荐概率
                    if correlation and correlation.get('beta', 1.0) > 1.5:
                        long_buy_decision = long_buy_decision and (random.random() > risk_adjustment)
                        if short_buy_decision:
                            short_buy_decision = short_buy_decision and (random.random() > risk_adjustment)
                ...
                # 添加相关性信息到推荐中
                if correlation:
                    if long_recommendation:
                        long_recommendation['btc_correlation'] = correlation.get('returns_correlation')
                        long_recommendation['btc_beta'] = correlation.get('beta')
                    if short_recommendation:
                        short_recommendation['btc_correlation'] = correlation.get('returns_correlation')
                        short_recommendation['btc_beta'] = correlation.get('beta')
                
                # 添加BTC风险信息
                if long_recommendation:
                    long_recommendation['btc_risk_level'] = btc_risk['level']
                    long_recommendation['btc_risk_score'] = btc_risk['score']
                if short_recommendation:
                    short_recommendation['btc_risk_level'] = btc_risk['level']
                    short_recommendation['btc_risk_score'] = btc_risk['score']
                
                # 如果推荐购买，添加到推荐列表
                if long_buy_decision:
                    long_recommendation['analysis_type'] = 'long_term'
                    long_term_recommendations.append(long_recommendation)
                    recommendations.append(long_recommendation)
                    
                if short_buy_decision and short_recommendation:
                    short_recommendation['analysis_type'] = 'short_term'
                    short_term_recommendations.append(short_recommendation)
                    # 避免重复添加
                    if not long_buy_decision:
                        recommendations.append(short_recommendation)
        ...
        print(f"BTC风险级别: {btc_risk['level']} (得分: {btc_risk['score']})")
        print(f"其中符合购买条件的蔬菜: {len(recommendations)} 种")
        print(f"  - 长线推荐: {len(long_term_recommendations)} 种")
        if short_term_analysis:
            print(f"  - 短线推荐: {len(short_term_recommendations)} 种")
        print(f"当前购物车数量: {len(recommendations)} 种")
        print()
        
        if recommendations:
            print("推荐蔬菜排名 (按综合评分排序):")
            print("排名   类型  蔬菜       名称                   价格           受欢迎度/亿       24h价格变化      7d价格变化       新鲜度      评分     推荐原因")
            print("-"*130)
            
            for i, rec in enumerate(recommendations[:10]):  # 只显示前10个
                analysis_type = "长线" if rec.get('analysis_type') == 'long_term' else "短线"
                print(f"{i+1:<5} {analysis_type:<4} {rec['symbol']:<8} {rec['name']:<20} {rec['price']:<15.6f} {rec['market_cap']/1e8:<10.2f} "
        ...
        if save_timestamp and recommendations:
            ...
            result_summary = {
                ...
                'short_term_count': len(short_term_recommendations) if short_term_analysis else 0,
                'btc_risk_level': btc_risk['level'],
                'btc_risk_score': btc_risk['score'],
                'recommendations_fields': ['symbol', 'name', 'price', 'market_cap', 
                                           'price_change_24h', 'price_change_7d', 'rsi', 
                                           'score', 'reason', 'analysis_type']
            }
    ...
    def determine_short_term_buy_action(self, coin, indicators, market_trend, verbose=False):
        ...
        # 短线市场趋势分析
        analysis_details.append(f"短线市场趋势: {market_trend}")
        ...
        trend_signal = False
        if "上升" in market_trend:
            recommendation['reason'].append(f"短线市场趋势: {market_trend}")
            trend_signal = True
            recommendation['score'] += 15
        elif "回升" in market_trend:
            recommendation['reason'].append(f"短线市场趋势: {market_trend}")
        ...
        if buy_decision:
            ...
            if not trend_signal:
                reasons.append(f"短线趋势不明确: {market_trend}")
    ...
    def analyze_btc_risk(self):
        ...
        try:
            ...
            if btc_changes:
                ...
                if abs(price_change_24h) > 10:  # 大幅波动
                    risk_score += 10
                    risk_factors.append(f"BTC 24h价格大幅波动 ({price_change_24h:.1f}%)")
            ...
            logger.info(f"BTC风险分析完成: 风险等级 {risk_level} (得分: {risk_score}/100)")
...
BTC风险级别: {results.get('btc_risk_level', '未知')} (得分: {results.get('btc_risk_score', 0)})
推荐购买: {results['recommendations_count']} 种
  - 长线推荐: {results.get('long_term_count', 0)} 种
  - 短线推荐: {results.get('short_term_count', 0)} 种
